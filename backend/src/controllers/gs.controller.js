...
const userClaimsCoins = catchAsync(async (req, res) => {
  try
  {
    ...
    await User.updateOne({userId:userId},{$inc:{coinsCollected:quantity}});
...
const buyItem = catchAsync(async (req, res) => {
  try {
    ...
    if(req.body.currencyId === "SOL") //This requires a different flow
    {
        payment = await gsService.CreateSolPayment(
        req.body.userId,
        itemToBuy);
    }
    else
